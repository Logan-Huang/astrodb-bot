...
- **Per-workflow artifact directories**: each workflow writes into its own directory in the project root — **`astrodb-build-artifacts/`** (parsed tables, generated schema YAML), **`astrodb-ingest-artifacts/`** (generated ingestion scripts, plus the ingest decision log `ingest-workflow.md`), and **`astrodb-website-artifacts/`**. Only the build workflow persists a `checklists.md` there (see Completion checklist below).
